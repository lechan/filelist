...
play = function(id,fn){
	var
	ua = BASE.ua,
	timing = BASE.countdown * 100,
	//interval = ua.indexOf('Android') == -1 ? 1 : 9,
	interval = 1,
	clock_con = $('#'+id);
	if(clock_con){
		clearInterval(timer);
		timer = setInterval(function(){
			...
			if(timing>=0){
				clock_con.html(doubleNum(Math.floor(timing/100))+"&prime;"+doubleNum(Math.floor(timing%100))+"&Prime;");
			}else{
				clearInterval(timer);
	...
	LENOVO.com.ajax.get('loginInfo.do','t='+new Date().getTime(), {"clientid":getclientid(),"lpsust":getLpsUst()}, function(data){
		if(data.status==true){
			...
			if(document.querySelector("#myPrize")){
				document.querySelector("#myPrize").innerHTML = '<a href="myprize.html"><img src="images/prize_btn.png" /></a>';
			}
...
init = function(id){
	LENOVO.com.ajax.get('loginInfo.do','t='+new Date().getTime(), {"clientid":getclientid(),"lpsust":getLpsUst()}, function(data){
		if(data.status==false){
			...
			if(document.querySelector("#myPrize")){
				document.querySelector("#myPrize").innerHTML = '<a href="javascript:LENOVO.com.lenovoid.login()"><img src="images/prize_btn.png" /></a>';
			}
			
		}else{
			if(document.querySelector("#login_box")){
				document.querySelector("#login_box").innerHTML = "<em>欢迎</em>"+formatName(data.username)+'，我的乐豆：<em id="ledou">'+data.hbNum+'</em>';
			}
...
LENOVO.win.pop = (function(){
	var poptpl = {
		...
		lottery_prize : '\
		<section class="pop_wrap">\
			<h2>亲！心愿梦想成真<br/>恭喜你获得#{name}</h2>\
			<div class="pop_img"><img src="#{img}" /></div>\
			<div class="btn_box">\
				<a class="btn" onClick="closePop()">稍后再领</a>\
				<a class="btn" onClick="prize_pop(this)" data-type="#{type}" data-prizeinfo="#{name}" data-logid="#{logid}">立即领取</a>\
			</div>\
		</section>\
		',
		lottery_ledou : '\
		<section class="pop_wrap">\
			<div class="pop_img"><img src="images/icon7.png" /></div>\
			<h2>恭喜亲！获得#{ledou}，乐豆可以在乐豆专区抽/兑礼物哟~</h2>\
			<div class="ledou_enter">\
				<a><img src="images/icon4.png" /></a>\
				<a><img src="images/icon5.png" /></a>\
			</div>\
			<div class="btn_box">\
				<a class="btn" onClick="closePop()">再来一次</a>\
			</div>\
		</section>\
		',
		lottery_fail : '\
		<section class="pop_wrap">\
			<div class="pop_img"><img src="images/icon6.png" /></div>\
			<h2>很遗憾，这次没有实现心愿，别灰心！</h2>\
			<div class="btn_box">\
				<a class="btn" onClick="closePop()">再来一次</a>\
			</div>\
		</section>\
		...
		loading : '\
		<section class="pop_wrap">\
			<h2 style="padding:20px 0px;"><img src="images/loading.gif" /></h2>\
		</section>\
		'
	},
	initEle = function(){
	...
	winpop = function(type,obj,info){
		...
		//提示弹窗
			...
			LENOVO.com.ui.dialog.open(poptpl.tips);	
		
		//loading弹窗
		}else if(type==9){
			LENOVO.com.ui.dialog.open(poptpl.loading);	
		}
	},
...
//改变乐豆数
function ledouChange(ledou){
	document.querySelector("#ledou").innerHTML = ledou;
}
...
//奖品弹窗
function prize_pop(e){
	var type = e.getAttribute('data-type');
	var prizeinfo = {
		'name' : e.getAttribute('data-prizeinfo'),
		'logId' : e.getAttribute('data-logid')
	}
	LENOVO.win.pop.init(type,{},prizeinfo);
}
...
//活动结束弹窗
function ending(){
	LENOVO.win.pop.init(8,{},{"msg":"活动已结束<br/>感谢您的参与"});	
}
